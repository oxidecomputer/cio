use chrono::{NaiveDateTime, offset::Utc, DateTime};
use reqwest::Url;
use httpmock::MockServer;
use serde_json::json;

use tailscale_api::{Tailscale, Device}; 
...
async fn list_devices_test() {
    ...
    assert_eq!(response[0], Device {
        addresses: vec![String::from("100.68.203.125")],
        allowed_ips: vec![],
        authorized: false,
        client_version: String::from("date.20201107"),
        created: DateTime::<Utc>::from_utc(NaiveDateTime::parse_from_str("2020-11-30T22:20:04Z", "%Y-%m-%dT%H:%M:%SZ").unwrap(), Utc),
        derp: String::from(""),
        display_node_key: String::from(""),
        endpoints: vec![],
        expires: DateTime::<Utc>::from_utc(NaiveDateTime::parse_from_str("2021-05-29T22:20:04Z", "%Y-%m-%dT%H:%M:%SZ").unwrap(), Utc),
        extra_ips: vec![],
        has_subnet: false,
        hostname: String::from("User1-Device"),
        id: String::from("12345"),
        is_external: false,
        last_seen: DateTime::<Utc>::from_utc(NaiveDateTime::parse_from_str("2020-11-30T17:20:04+00:00", "%Y-%m-%dT%H:%M:%S%z").unwrap(), Utc), 
        log_id: String::from(""),
