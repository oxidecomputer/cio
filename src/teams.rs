...
pub fn cmd_teams_run(cli_matches: &ArgMatches) {
    ...
    handlebars
        .register_helper("terraformize", Box::new(terraform_username_helper));
...
resource "aws_iam_policy" "policy-{{terraformize this.username}}" {
  name        = "policy-{{terraformize this.username}}"
  path        = "/"
  description = "Allows {{terraformize this.username}} to manage their own credentials."
...
resource "aws_iam_policy_attachment" "attach-{{terraformize this.username}}" {
  name       = "attachment-{{terraformize this.username}}"
  users      = [aws_iam_user.{{terraformize this.username}}.name]
  policy_arn = aws_iam_policy.policy-{{terraformize this.username}}.arn
}
{{/if}}{{/each}}
"#;
