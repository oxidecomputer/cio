...
pub async fn refresh_sf_leads(db: &Database, company: &Company) -> Result<()> {
    ...
    // Skip any subscribers without a company field
    let has_company = rack_line_subscribers::dsl::company.ne("".to_string());
            ...
            if !subscriber.name.is_empty() {
                ...
                if name_parts.peek().is_some() {
                    ...
                    if !last_name.is_empty() {
                        ...
                        if REMOTE_ACTIVE {
                            log::info!(
                                "Upserting lead against Airtable_Lead_Record_Id__c {}",
