...
impl NewSwagInventoryItem {
    ...
    pub async fn generate_barcode_images(
        ...
        self.barcode_pdf_label.to_string()
    }

    pub async fn expand(&mut self, drive_client: &GoogleDrive, drive_id: &str, parent_id: &str) -> String {
...
pub fn generate_pdf_barcode_label(
    ...
    let barcode_image = PdfImage::from_dynamic_image(&image::load_from_memory(image_bytes).unwrap());
